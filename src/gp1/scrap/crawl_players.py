...
def getPlayerUsingRegEx(directory):
    ...
    def getPlayersForAFilenameUsingRegex(filename):
        ...
        listOfDictionaries = []
        html = fileHandle.read()
        table = re.search(tableRegex, html, re.VERBOSE | re.DOTALL)
        if table:
            tableContents = table.group(1)
        else:
            return
        alltrs = re.findall(allTrsRegex, tableContents, re.VERBOSE | re.DOTALL)
        if len(alltrs) > 0: #Is there any table at all?? http://www.basketball-reference.com/players/x doesnt have it
            alltrs = alltrs[0]
