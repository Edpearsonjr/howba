...
sqlStatement  <- "SELECT * FROM SF_PREVIOUS5YEARS_AVERAGE"
...
for(i in 1:finalNumberOfClusters){
    averageScore <- sum( (0.6* membersOfClusters[[i]]$POINTS) + (0.2 * membersOfClusters[[i]]$DEFENSIVE_REBOUNDS) + (0.1 * membersOfClusters[[i]]$OFFENSIVE_REBOUNDS) + (0.1)* membersOfClusters[[i]]$BLOCKS   )/ length(membersOfClusters[[i]])
